...
//#endif
    ...
    public class LUMINA_FAR_A : MonoBehaviour
    {
        //v1.6
        ...
        void Init()
        {
            ...
            voxelizationShader = Shader.Find("SEGI/SEGIVoxelizeScene_FAR_A");
            voxelTracingShader = Shader.Find("SEGI/SEGITraceScene_FAR_A");
            ...
            if (!material)
            {
                material = new Material(Shader.Find("Hidden/SEGI_FAR_A"));
            ...
            GameObject vcgo = GameObject.Find("SEGI_VOXEL_CAMERA_FAR_A");

            if (!vcgo)
            {
                voxelCameraGO = new GameObject("SEGI_VOXEL_CAMERA_FAR_A");
            ...
            GameObject lvp = GameObject.Find("SEGI_LEFT_VOXEL_VIEW_FAR_A");

            if (!lvp)
            {
                leftViewPoint = new GameObject("SEGI_LEFT_VOXEL_VIEW_FAR_A");
            ...
            GameObject tvp = GameObject.Find("SEGI_TOP_VOXEL_VIEW_FAR_A");

            if (!tvp)
            {
                topViewPoint = new GameObject("SEGI_TOP_VOXEL_VIEW_FAR_A");
...
//                }
        ...
        void ExecuteBeforeCameraRender(ScriptableRenderContext context, Camera camera) //void OnPreRender()
        {
            ...
            if (renderState == RenderState.Voxelize)
            {
                ...
                Shader.SetGlobalMatrix("SEGIVoxelViewFront_FAR_A", TransformViewMatrix(voxelCamera.transform.worldToLocalMatrix));
                Shader.SetGlobalMatrix("SEGIVoxelViewLeft_FAR_A", TransformViewMatrix(leftViewPoint.transform.worldToLocalMatrix));
                Shader.SetGlobalMatrix("SEGIVoxelViewTop_FAR_A", TransformViewMatrix(topViewPoint.transform.worldToLocalMatrix));
