...
{
    public class VolumetricLightedSEGI_FAR_B : ScriptableRendererFeature
    {
        [System.Serializable]
        ...
        class LightScatteringPass : ScriptableRenderPass
        {
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                if (Camera.main != null)
                {
                    LUMINA_FAR_B segi = Camera.main.GetComponent<LUMINA_FAR_B>();
                    if (segi != null)
                    {
                    ...
                    if (cameraData.camera == Camera.main)
                    {
                        ...
                        if (segi != null && segi.enabled)
                        {
                            ...
                            //Set parameters
                            Shader.SetGlobalFloat("SEGIVoxelScaleFactor_FAR_B", segi.voxelScaleFactor);
     ...
     /////                    cmd.Blit(source, currentDepth, segi.material, LUMINA.Pass.GetCameraDepthTexture);//v0.1
                            ...
                            using (var builder = renderGraph.AddRasterRenderPass<PassData>(passName, out var passData))
                            {
                                ...
                                builder.SetRenderFunc((PassData data, RasterGraphContext context) =>
                                    ...
                                    "PreviousGITexture_FAR_B", previousGIResult,
                                    "PreviousGITexture_FAR_B", previousGIResult,
           ...
           //     ConfigureTarget(_occluders.Identifier());//v0.1
            ...
            void OnRenderImage(Camera camera, CommandBuffer cmd, RenderTexture source, RenderTexture destination)
            {
                ...
                if (Camera.main != null)
                {
                    ...
                    if (segi != null && segi.enabled)
                    {
		     		...
		     		cmd.Blit( source, destination);
                        ...
                        //Set parameters
                        Shader.SetGlobalFloat("SEGIVoxelScaleFactor_FAR_B", segi.voxelScaleFactor);//Shader.SetGlobalFloat("SEGIVoxelScaleFactor", segi.voxelScaleFactor);
		     ...
		     cmd.Blit( currentNormal, destination);
                        ...
                        //Set the previous GI result and camera depth textures to access them in the shader
                        segi.material.SetTexture("PreviousGITexture_FAR_B", segi.previousGIResult);

                        Shader.SetGlobalTexture("PreviousGITexture_FAR_B", segi.previousGIResult);// Shader.SetGlobalTexture("PreviousGITexture", segi.previousGIResult);
