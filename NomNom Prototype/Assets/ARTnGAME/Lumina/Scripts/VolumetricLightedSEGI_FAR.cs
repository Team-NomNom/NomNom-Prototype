...
{
    public class VolumetricLightedSEGI_FAR : ScriptableRendererFeature
    {
        [System.Serializable]
        ...
        class LightScatteringPass : ScriptableRenderPass
        {
            ...
            public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
            {
                ...
                if (Camera.main != null)
                {
                    LUMINA_FAR segi = Camera.main.GetComponent<LUMINA_FAR>();
                    if (segi != null)
                    {
                    ...
                    Shader.SetGlobalInt("_visibleLightsCount", cullResults.visibleLights.Length); //v1.6
                    ...
                    if (cameraData.camera == Camera.main)
                    {
                        ...
                        if (segi != null && segi.enabled)
                        {
                            ...
                            //Set parameters
                            Shader.SetGlobalFloat("SEGIVoxelScaleFactor_FAR", segi.voxelScaleFactor);
     ...
     /////                    cmd.Blit(source, currentDepth, segi.material, LUMINA.Pass.GetCameraDepthTexture);//v0.1
                            ...
                            using (var builder = renderGraph.AddRasterRenderPass<PassData>(passName, out var passData))
                            {
                                ...
                                builder.SetRenderFunc((PassData data, RasterGraphContext context) =>
                                    ...
                                    "PreviousGITexture_FAR", previousGIResult,
                                    "PreviousGITexture_FAR", previousGIResult,
           ...
           //     ConfigureTarget(_occluders.Identifier());//v0.1
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                if (Camera.main != null)
                {
                    // Camera.main.TryGetCullingParameters(out ScriptableCullingParameters cullingParameters);
                    Shader.SetGlobalInt("_visibleLightsCount", renderingData.cullResults.visibleLights.Length); //v1.6
                    //Debug.Log(renderingData.cullResults.visibleLights.Length);
                }
		     ...
		     cmd.Blit(source, skyA);
                    ...
                    //{
                    //    //Debug.Log("FAR = " + cameraA.targetTexture.name);
                    //    // cmd.Blit(gi1, source);
                    //    cmd.SetRenderTarget(cameraA.targetTexture);
                    //    cmd.Blit(gi1, cameraA.targetTexture);
                    //}
                    //else
                    //{
                    //    cmd.Blit(gi1, source);// _cameraColorTargetIdent); //1.0g
            ...
            void OnRenderImage(Camera camera, CommandBuffer cmd, RenderTexture source, RenderTexture destination)
            {
                ...
                if (Camera.main != null)
                {
                    ...
                    if (segi != null && segi.enabled)
                    {
		     		...
		     		cmd.Blit( source, destination);
                        ...
                        //Set parameters
                        Shader.SetGlobalFloat("SEGIVoxelScaleFactor_FAR", segi.voxelScaleFactor);//Shader.SetGlobalFloat("SEGIVoxelScaleFactor", segi.voxelScaleFactor);
		     ...
		     cmd.Blit( currentNormal, destination);
                        ...
                        //Set the previous GI result and camera depth textures to access them in the shader
                        segi.material.SetTexture("PreviousGITexture_FAR", segi.previousGIResult);

                        Shader.SetGlobalTexture("PreviousGITexture_FAR", segi.previousGIResult);// Shader.SetGlobalTexture("PreviousGITexture", segi.previousGIResult);
