...
sampler2D PreviousGITexture_FAR_A;
float SEGIVoxelScaleFactor_FAR_A;
	...
	Pass //0
	{
		CGPROGRAM
			...
			float4 frag(v2f input) : SV_Target
			{
				...
				float3 voxelOrigin = voxelSpacePosition.xyz + worldNormal.xyz * 0.003 * ConeTraceBias * 1.25 / SEGIVoxelScaleFactor_FAR_A;
	...
	Pass //3 Temporal blend (with unity motion vectors)
	{
		CGPROGRAM
			...
			float4 frag(v2f input) : COLOR0
			{
				...
				float3 prevGI = tex2Dlod(PreviousGITexture_FAR_A, float4(reprojCoord, 0.0, 0.0)).rgb;
	...
	Pass //4 Specular/reflections trace
	{
		...
		CGPROGRAM
			...
			float4 frag(v2f input) : SV_Target
			{
				...
				float3 voxelOrigin = voxelSpacePosition.xyz + worldNormal.xyz * 0.006 * ConeTraceBias * 1.25 / SEGIVoxelScaleFactor_FAR_A;
				...
				voxelOrigin += worldNormal.xyz * 0.002 * 1.25 / SEGIVoxelScaleFactor_FAR_A;
	...
	Pass //12 Temporal blending without motion vectors (for legacy support)
	{
		CGPROGRAM
			...
			float4 frag(v2f input) : COLOR0
			{
				...
				float3 prevGI = tex2D(PreviousGITexture_FAR_A, reprojCoord).rgb;
