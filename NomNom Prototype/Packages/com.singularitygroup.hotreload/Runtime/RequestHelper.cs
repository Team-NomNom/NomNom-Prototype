    ...
    public static class RequestHelper {
        ...
        internal static async void PollMethodPatches(Action<MethodPatchResponse> onResponseReceived) {
            ...
            try {
                ...
                if(result.statusCode == HttpStatusCode.OK) {
                    ...
                    Debug.Log($"PollMethodPatches failed with code {(int)result.statusCode} {result.responseText} {result.exception}");
                }
            } finally {
                pollPending = false;
            }
        ...
        public static async Task KillServer() {
            ...
            try {
                ...
                using(await client.PostAsync(CreateUrl(serverInfo) + "/kill", new ByteArrayContent(Array.Empty<byte>())).ConfigureAwait(false)) { }
            } catch {
                //ignored
            } finally {
                client.Dispose();
