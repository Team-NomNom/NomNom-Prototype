        ...
        private static void InvokeStaticMethod(MethodInfo m) {
            try {
                ...
                if (m.GetParameters().Length != 0) {
                    Debug.LogException(new AggregateException($"[InvokeOnHotReload] {m.DeclaringType?.Name} {m.Name} failed. Make sure it has 0 parameters", e));
                } else {
                    Debug.LogException(new AggregateException($"[InvokeOnHotReload] {m.DeclaringType?.Name} {m.Name} failed", e));
                }
            }
        }
