...
namespace SingularityGroup.HotReload.Editor.Cli {
    internal static class CliUtils {
        public static string GetTempDownloadFilePath() {
            if (UnityHelper.Platform == RuntimePlatform.OSXEditor) {
                ...
                return Path.GetFullPath(PackageConst.LibraryCachePath + "/HotReloadServerTemp/Server.tmp");
            } else {
...
#           else
        ...
        public static string GetExecutableTargetDir() {
            return Path.Combine(GetAppDataPath(), $"executables_{PackageConst.Version.Replace('.', '-')}");
        }
