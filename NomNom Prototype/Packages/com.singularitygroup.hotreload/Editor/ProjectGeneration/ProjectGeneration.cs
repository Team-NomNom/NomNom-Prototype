...
#               else
            ...
            await ThreadUtility.SwitchToThreadPool();
            for (var i = 0; i < projectParts.Count; i++) {
                SyncProject(projectParts[i], responseFiles[i], filteredProjects);
            }
            ...
            foreach (var pp in m_PostProcessors) {
                try {
                    ...
                    Debug.LogWarning($"Post processor '{pp}' threw exception when calling OnGeneratedCSProjectFilesThreaded:\n{ex}");
                }
            }
        }
        ...
        private void SyncProject(
            ...
            SyncProjectFileIfNotChanged(
                ProjectFile(island),
                ProjectText(island, responseFileData, filteredProjects));
        }
        ...
        private void SyncProjectFileIfNotChanged(string path, string newContents) {
            foreach (var pp in m_PostProcessors) {
                try {
                    ...
                    Debug.LogWarning($"Post processor '{pp}' failed when processing project '{path}':\n{ex}");
                }
            }
        ...
        private void SyncSolutionFileIfNotChanged(string path, string newContents) {
            foreach (var pp in m_PostProcessors) {
                try {
                    ...
                    Debug.LogWarning($"Post processor '{pp}' failed when processing solution '{path}':\n{ex}");
                }
            }
        ...
        private string ProjectText(ProjectPart assembly, List<ResponseFileData> responseFilesData, HashSet<string> filteredProjects) {
            ...
            m_PostProcessors = await Task.Run(() => {
                ...
                foreach (var type in types) {
                    try {
                        ...
                        Debug.LogWarning($"The type '{type.FullName}' was expected to have a public default constructor but it didn't");
                    } catch (TargetInvocationException ex) {
                        Debug.LogWarning($"Exception occurred when invoking default constructor of '{type.FullName}':\n{ex.InnerException}");
                    } catch (Exception ex) {
                        Debug.LogWarning($"Unknown exception encountered when trying to create post processor '{type.FullName}':\n{ex}");
                    }
                }
